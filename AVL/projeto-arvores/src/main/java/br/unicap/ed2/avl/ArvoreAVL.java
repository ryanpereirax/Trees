...
public class ArvoreAVL {
    ...
    public void inserir(int noAVL) {
        ...
        Finserir(this.raiz, n);
    }

    // declara que um nó e raiz e passa para o getData
    public boolean isRoot(int i) {
        if (raiz == null) {
            return false;
        }
        return raiz.getData() == i;
    }

    // descrição da função de inserir
    public void Finserir(NoAVL a, NoAVL b) {
        if (a == null) {
            raiz = b;
        } else {
            if (b.getData() < a.getData()) {
                if (a.getEsquerda() == null) {
                    ...
                    Finserir(a.getEsquerda(), b);
                }
            } else if (b.getData() > a.getData()) {
                if (a.getDireita() == null) {
                    a.setDireita(b);
                    b.setPai(a);
                    balancear(a);
                } else {
                    Finserir(a.getDireita(), b);
                }
            } else if (b.getData() == a.getData()) {
                if (a.getDireita() == null) {
                    a.setDireita(b);
    ...
    // função que imprime
    public void imprimir(String prefix, NoAVL no, boolean isLeft) {

        if (no != null) {
            imprimir(prefix + "    ", no.getDireita(), false);
            System.out.println(prefix + ("|--") + no.getData());
            imprimir(prefix + "    ", no.getEsquerda(), true);
        }
    ...
    public void Fdeletar(NoAVL no, int i) {
        // getData e a raiz
        if (no != null) {
            if (no.getData() > i) {
                Fdeletar(no.getEsquerda(), i);

            } else if (no.getData() < i) {
                Fdeletar(no.getDireita(), i);
